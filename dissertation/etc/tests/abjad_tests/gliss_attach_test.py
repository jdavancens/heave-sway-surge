...
f(staff)
print(inspect_(staff).get_grace_containers(kind='after'))

# pairwise = sequencetools.iterate_sequence_nwise
# for current, next in pairwise(iterate(staff).by_logical_tie()):
#     grace_container = inspect_(current.tail).get_grace_container(kind='after')
...
#     leaf_1 = leaf_pair[0]
#     leaf_2 = leaf_pair[1]
#     logical_tie_1 = inspect_(leaf_1).get_logical_tie()
#     logical_tie_2 = inspect_(leaf_2).get_logical_tie()
#     text_spanner = spannertools.TextSpanner()
