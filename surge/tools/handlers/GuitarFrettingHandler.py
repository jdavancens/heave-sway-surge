...
class GuitarFrettingHandler(TablatureHandler):
    ...
    def __init__(
        ...
        self._hammer_patterns = self._create_cursors(hammer_patterns)
        self._glissando_patterns = self._create_cursors(glissando_patterns)
    ...
    def _attach_hammer(self, hammer, tie):
        if hammer is not None:
            accent = indicatortools.Articulation("ltoe")
    ...
    def _handle_lifeline_voice(self, voice, current_stage):
        ...
        # reset the fret combination pattern cursor
        cursor = self._fret_combination_patterns[current_stage]
        self._fret_combination_patterns[current_stage] = self._reset_cursor(cursor)
        ...
        for logical_tie in iterate(voice).by_logical_tie(pitched=True):
            ...
            fret_combination_index = self._cursor_next(self._fret_combination_patterns, current_stage)
            fret_combination = self._fret_combinations[current_stage][fret_combination_index]
    ...
    def _handle_rhythm_voice(self, rhythm_voice, current_stage):
        for tie, _, _ in self._iterate_logical_ties(rhythm_voice):
            hammer = self._cursor_next(self._hammer_patterns, current_stage)
