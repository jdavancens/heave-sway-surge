...
class TestBowingHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        height_envelopes = [
            surge.tools.bezzie.Path(
                surge.tools.bezzie.BezierCurve((0, 0), (1, 1))
            )
        ]
        pressure_envelopes = [
            surge.tools.bezzie.Path(
                surge.tools.bezzie.BezierCurve((0, 1), (1, 0))
            )
        ]
        ...
        music_maker = surge.tools.makers.MusicMaker(
        ...
        self.handler = surge.tools.handlers.BowingHandler(
