...
class Handler(object):
    ...
    def _make_circle_outline_markup(self, size):
        circle_outline = abjad.markuptools.MarkupCommand(
            'draw-circle',
            size,
            0.1,
            False
        )
        return abjad.markuptools.Markup(circle_outline)
    ...
    def _make_text_markup(
        ...
        if orientation is 'x':
            ...
            markup = abjad.markuptools.Markup.column(
                text_or_tuple,
                direction=direction
            )
        if enclosure is 'box':
