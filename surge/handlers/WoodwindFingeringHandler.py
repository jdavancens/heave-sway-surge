...
class WoodwindFingeringHandler(TablatureHandler):
    ...
    def _make_key_name_markup(self, key_name):
        ...
        if key_name.lower() in pitch:
            if len(key_name) > 1:
                p = key_name[0]
                p = abjad.markuptools.Markup(p.upper())
                acc = key_name[1:]
                if acc == 'es':
                    acc = abjad.markuptools.Markup.flat().raise_(0.5)
                else:
                    acc = abjad.markuptools.Markup.sharp().raise_(0.5)
                markup = abjad.markuptools.Markup.concat([p, acc])
            else:
                markup = abjad.markuptools.Markup(key_name.upper())
