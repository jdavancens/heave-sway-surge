...
class TestEmbouchureHandler(unittest.TestCase):

    def setUp(self):
        time_signatures = [[abjad.TimeSignature((4, 4))]]
        air_pressure_envelopes = [
            Path(BezierCurve((0, 0), (1, 1)))]
        lip_pressure_envelopes = [
            Path(BezierCurve((0, 1), (1, 0)))]
        ...
        self.handler = EmbouchureHandler(
    ...
    def test__attach_direction(self):
        voice = abjad.Voice("c'4")
        for tie in abjad.iterate(voice).by_logical_tie(pitched=True):
            self.handler._attach_direction('in', 'out', tie)

    def test__attach_fluttertongue(self):
        voice = abjad.Voice("c'4")
        for tie in abjad.iterate(voice).by_logical_tie(pitched=True):
            self.handler._attach_fluttertongue(True, tie)

    def _attach_phoneme(self):
        voice = abjad.Voice("c'4")
        for tie in abjad.iterate(voice).by_logical_tie(pitched=True):
            self.handler._attach_phoneme('k', 'u', tie)
