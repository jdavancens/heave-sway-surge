...
def split(counts):
    counts_1 = sequencetools.sum_elements(counts, [(0, 2)], period=2)
    counts_2 = [counts[0]] + \
        sequencetools.sum_elements(counts[1:], [(0, 2)], period=2)
...
taleas_picking_guitar_1 = {
    '2': rhythmmakertools.Talea(
        counts=split(counts_by_stage[0])[0],
        denominator=16
    ),
    '3': rhythmmakertools.Talea(
        counts=split(counts_by_stage[1])[0],
        denominator=16
    ),
    '4': rhythmmakertools.Talea(
        counts=split(counts_by_stage[2])[0],
        denominator=16
    ),
    '5': rhythmmakertools.Talea(
