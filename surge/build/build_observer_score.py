# -*- coding: utf-8 -*-
from surge.segments.observer.make_score import make_score
import abjad
import datetime
import os
import sys
...
if __name__ == '__main__':
    with abjad.systemtools.Timer() as big_timer:
        ...
        score_ly_path = os.path.join(build_path, 'observer_score.ly')
        score_pdf_path = os.path.join(build_path, 'observer_score.pdf')
        ...
        with abjad.systemtools.Timer() as timer:
            ...
            abjad.systemtools.IOManager.run_lilypond(score_ly_path, flags='-V')
            seconds = int(timer.elapsed_time)
            time = str(datetime.timedelta(seconds=seconds))
            print(time)
            print("...Done")
