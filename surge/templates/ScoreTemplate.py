...
class ScoreTemplate:
    ...
    def _create_staff_group(self, instrument_name, name, short_name):
        if instrument_name in instrument_categories['woodwinds']:
            instrument = instruments[instrument_name]
            template = surge.templates.WoodwindStaffGroupTemplate(
                instrument
            )
            staff_group = template()
        elif instrument_name in instrument_categories['trombone']:
            instrument = instruments[instrument_name]
            template = surge.templates.TromboneStaffGroupTemplate(
                instrument
            )
            staff_group = template()
        elif instrument_name in instrument_categories['plucked strings']:
            instrument = instruments[instrument_name]
            template = surge.templates.GuitarStaffGroupTemplate(
                instrument
            )
            staff_group = template()
        elif instrument_name in instrument_categories['bowed strings']:
            instrument = instruments[instrument_name]
            template = surge.templates.BowedStringStaffGroupTemplate(
                instrument
            )
            staff_group = template()
        else:
        ...
        abjad.setting(
            staff_group
        ).instrument_name = abjad.marktuptools.Markup(name)
        abjad.setting(
            staff_group
        ).short_instrument_name = abjad.markuptools.Markup(short_name)
        return staff_group
