...
# from materials.staff_map import staff_map
from surge.templates.ScoreTemplate import ScoreTemplate
from surge.makers.SegmentMakerBaseClass \
...
class SegmentMaker(SegmentMakerBaseClass):
    ...
    def _annotate_stages(self):
        ...
        for stage_index in range(self.number_of_stages):
            ...
            attach(rehearsal_mark, start_measure[0])
    ...
    def _interpret_music_handlers(self):
        ...
        for stage in range(self.number_of_stages):
            ...
            for music_handler in self._music_handlers:
                ...
                for voice in voices:
                    voice_instrument = inspect(voice).get_indicator(
                        Instrument)
                    ...
                    for staff in abjad.iterate(
                        ...
                        if staff.name is 'Separator':
                            continue
                        staff_instrument = abjad.inspect(
                            staff
                        ).get_annotation('instrument')
    ...
    def _make_instrument_change_markup(self, instrument_name):
        ...
        markup = abjad.markuptools.Markup(string, direction=1)
    ...
    def _make_repeated_note_measures(self):
        ...
        for stage in self.time_signatures[0:self.number_of_stages]:
            flattened_list = abjad.sequencetools.flatten_sequence(stage)
            time_signatures.extend(flattened_list)
        measures = []
        for time_signature in time_signatures:
            notes = abjad.scoretools.make_repeated_notes_from_time_signature(
                time_signature
            )
    ...
    def _populate_time_signature_contexts(self):
        ...
        leaves = abjad.iterate(
            measures
        ).by_classa(abjad.scoretools.Leaf)
        leaves = list(leaves)
        first_leaf = leaves[0]
        ...
        voice = abjad.scoretools.Voice(measures)
        time_signature_context.append(voice)

        for staff in abjad.iterate(
            self._score
        ).by_class(abjad.scoretools.Staff):
            if staff.name is 'Separator':
                measures = self._make_skip_filled_measures()
    ...
    def _raise_approximate_duration_in_seconds(self):
        ...
        leaves = abjad.iterate(
            context
        ).by_class(abjad.scoretools.Leaf)
        measure_summaries = []
        tempo_index = 0
        is_trending = False
